...
A seguir observamos a distribuição de usuários dessa linguagem ao longo do tempo.
```{r}
...
A linha do tempo de modificações em Markdown é a seguinte:
```{r}
dados_md_m <- dados_md %>%
  select(-c(file_extension, fds, dia_da_semana)) %>%
  remove_rownames %>% 
...
highchart(type = "stock") %>% 
  hc_title(text = "Timeline da Atividade de Markdown") %>%
  hc_add_series(dados_md_m)
```
...
data.frame(sd.diff.ci) %>% 
  ...
  geom_hline(yintercept = 0, colour = "darkblue")
```
Usamos o desvio padrão para comparar a variação das duas linguagens, e temos com 95% de confiança que a diferença do desvio padrão de cada linguagem está entre `r sd.diff.ci`. Esse intervalo está bastante longe do zero, logo podemos afirmar com o grau de confiança já citado que nos finais de semana há mais variação entre os desenvolveres em JavaScript do que para os que desenvolvem em Markdown.

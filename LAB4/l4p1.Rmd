...
Essa visualização exibe, com 95% de confiança, a diferença das medianas entre a atividade no fim de semana e nos dias úteis. O intervalo é de `r median.md.diff`. Como esse intervalo inclui o zero, não é possível afirmar com segurança que Markdown é mais usado na semana ou no fim de semana.  
...
data.frame(median.js.diff) %>% 
  ...
  geom_hline(yintercept = 0, colour = "darkblue")
```
No caso de JavaScript a diferença das medianas com 95% de confiança está entre `r median.js.diff` o que, como no caso anterior, inclui o zero, então não é possível determinar a existência de uma diferença significativa de commits nos períodos estudados.
...
median.java <- bootstrap2(dados_java$users,
                        treatment = dados_java$fds,
                        median,
                        R = 10000)

median.java.diff <- CI.percentile(median.java, probs = c(.025, .975))

data.frame(median.java.diff) %>% 
  ggplot(aes(x = "Diferença", ymin = X2.5., ymax = X97.5.)) + 
  geom_errorbar(width = .2) + 
  geom_hline(yintercept = 0, colour = "darkblue")
```
